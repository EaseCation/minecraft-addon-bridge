...
public sealed interface BeFiltersSpec {
    @JsonIgnoreProperties(ignoreUnknown = true) 
    record BeFiltersSpec_Variant0(
        /* All tests in an {@code all_of} group must pass in order for the group to pass. */
        @JsonProperty("all_of") @Nullable BeGroupsSpec allOf,
        /* One or more tests in an {@code any_of} group must pass in order for the group to pass. */
        @JsonProperty("any_of") @Nullable BeGroupsSpec anyOf,
        /* All tests in a {@code none_of} group must fail in order for the group to pass. */
        @JsonProperty("none_of") @Nullable BeGroupsSpec noneOf
    ) implements BeFiltersSpec {
    }
    @JsonIgnoreProperties(ignoreUnknown = true) 
    record BeFiltersSpec_Variant1(
    ) implements BeFiltersSpec {
    }
}
